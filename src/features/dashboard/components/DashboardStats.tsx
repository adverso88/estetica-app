...
export function DashboardStats({ stats, userRole }: DashboardStatsProps) {
  const cards = [
    ...
    {
      title: userRole === 'profesional' ? 'Ingresos del Mes' : 'Completadas',
      value: userRole === 'profesional' ? `$${stats.monthlyRevenue.toLocaleString()}` : stats.completedAppointments,
      icon: userRole === 'profesional' ? (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
