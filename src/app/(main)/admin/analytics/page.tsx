import { createClient } from '@/lib/supabase/server'
import { redirect } from 'next/navigation'
import { Card } from '@/components/ui/card'
import { AnalyticsDashboard } from './AnalyticsDashboard'

export const metadata = {
  title: 'Analytics | EstéticaApp Admin'
}

export default async function AdminAnalyticsPage() {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()

  if (!user) {
    redirect('/login')
  }

  // Verificar que sea admin
  const { data: profile } = await supabase
    .from('profiles')
    .select('role')
    .eq('id', user.id)
    .single()

  if (profile?.role !== 'master') {
    redirect('/dashboard')
  }

  // Calcular fechas
  const now = new Date()
  const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1)
  const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0)
  const startOfLastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1)
  const endOfLastMonth = new Date(now.getFullYear(), now.getMonth(), 0)

  // Obtener citas del mes actual
  const { data: currentMonthAppointments } = await supabase
    .from('citas')
    .select('*, tratamiento:tratamientos(precio)')
    .gte('fecha_hora', startOfMonth.toISOString())
    .lte('fecha_hora', endOfMonth.toISOString())

  // Obtener citas del mes anterior
  const { data: lastMonthAppointments } = await supabase
    .from('citas')
    .select('*, tratamiento:tratamientos(precio)')
    .gte('fecha_hora', startOfLastMonth.toISOString())
    .lte('fecha_hora', endOfLastMonth.toISOString())

  // Obtener pacientes nuevos del mes
  const { data: newClients } = await supabase
    .from('pacientes')
    .select('id')
    .gte('created_at', startOfMonth.toISOString())

  const { data: lastMonthClients } = await supabase
    .from('pacientes')
    .select('id')
    .gte('created_at', startOfLastMonth.toISOString())
    .lte('created_at', endOfLastMonth.toISOString())

  // Obtener especialistas con sus citas
  const { data: profesionales } = await supabase
    .from('profesionales')
    .select(`
      id,
      tarifa_hora,
      nombre,
      citas(id, estado, fecha_hora, duracion_minutos, tratamiento:tratamientos(precio))
    `)
    .eq('is_active', true)

  // Calcular métricas
  const currentAppts = currentMonthAppointments || []
  const lastAppts = lastMonthAppointments || []

  // Ingresos
  const currentRevenue = currentAppts
    .filter(a => a.estado === 'completada' || a.estado === 'confirmada') // Ajustar según estados reales
    .reduce((sum, a) => sum + (Number(a.precio) || Number(a.tratamiento?.precio) || 0), 0)

  const lastRevenue = lastAppts
    .filter(a => a.estado === 'completada' || a.estado === 'confirmada')
    .reduce((sum, a) => sum + (Number(a.precio) || Number(a.tratamiento?.precio) || 0), 0)

  // Cuentas por cobrar (agendadas o confirmadas pero no completadas/pagadas)
  const accountsReceivable = currentAppts
    .filter(a => a.estado === 'agendada' || a.estado === 'confirmada')
    .reduce((sum, a) => sum + (Number(a.precio) || Number(a.tratamiento?.precio) || 0), 0)

  // Tasa de conversión (citas asistidas/completadas)
  const completedAppointments = currentAppts.filter(a => a.estado === 'completada').length
  const totalAppointments = currentAppts.length
  const conversionRate = totalAppointments > 0 ? (completedAppointments / totalAppointments) * 100 : 0

  // Métricas por especialista
  const lawyerMetrics = (profesionales || []).map((pro: any) => {
    const proAppts = pro.citas || []
    const monthAppts = proAppts.filter((a: any) => {
      const date = new Date(a.fecha_hora)
      return date >= startOfMonth && date <= endOfMonth
    })
    const completedAppts = monthAppts.filter((a: any) => a.estado === 'completada')

    const revenue = completedAppts.reduce((sum: number, a: any) =>
      sum + (Number(a.precio) || Number(a.tratamiento?.precio) || 0), 0
    )

    const hoursWorked = completedAppts.reduce((sum: number, a: any) =>
      sum + ((a.duracion_minutos || 60) / 60), 0
    )
    const utilizationRate = hoursWorked > 0 ? (hoursWorked / 160) * 100 : 0

    return {
      id: pro.id,
      name: pro.nombre || 'Sin nombre',
      appointments: monthAppts.length,
      completed: completedAppts.length,
      revenue,
      hoursWorked: Math.round(hoursWorked * 10) / 10,
      utilizationRate: Math.round(utilizationRate),
      hourlyRate: pro.tarifa_hora,
    }
  })

  const stats = {
    revenue: {
      current: currentRevenue,
      previous: lastRevenue,
      change: lastRevenue > 0 ? ((currentRevenue - lastRevenue) / lastRevenue) * 100 : 0,
    },
    appointments: {
      current: currentAppts.length,
      previous: lastAppts.length,
      change: lastAppts.length > 0 ? ((currentAppts.length - lastAppts.length) / lastAppts.length) * 100 : 0,
    },
    clients: {
      new: newClients?.length || 0,
      previousNew: lastMonthClients?.length || 0,
    },
    conversionRate,
    accountsReceivable,
    lawyerMetrics,
  }

  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      <AnalyticsDashboard stats={stats} />
    </div>
  )
}
