                ...
                {plantillas?.map((p) => (
                    <div key={p.id} className="card-elevated p-6 hover:shadow-lg transition-all group">
                        <div className="flex items-start justify-between mb-4">
                            ...
                            <span className="text-[10px] bg-secondary-50 text-secondary-600 px-2 py-1 rounded-full font-bold uppercase tracking-wider">v{p.version}</span>
                        </div>

                        <h3 className="font-heading font-bold text-xl text-foreground mb-2 group-hover:text-primary-600 transition-colors">{p.titulo}</h3>
                        <p className="text-sm text-foreground-secondary line-clamp-3 mb-6">
                            {p.contenido.replace(/[#*]/g, '').slice(0, 150)}...
                        </p>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-border-light mt-auto">
                            ...
                            <button className="btn-ghost text-[11px] py-1 px-3">Previsualizar</button>
                        </div>
                    </div>
                ))}
